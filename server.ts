...
// Handles passive consumption, bot simulation, consumption rates, and debriefing snapshots
setInterval(() => {
  const now = Date.now();
  ...
  if (gameState.status === 'playing') {
    ...
    for (const tid of Object.keys(gameState.teams)) {
      ...
      if (gameState.isBotEnabled) {
        // Random clicks: ~3-8 clicks per second per team
        if (Math.random() < 0.45) {
    ...
    // 4. Save Debriefing History Snapshot every 1.5 seconds
    if (now - lastHistoryRecordTime >= 1500) {
      ...
      // Cap history to 600 items (~15 min of play)
      if (gameState.history.length > 600) {
        gameState.history.shift();
      }
    }

    // Broadcast live state updates to all clients
    broadcastState();
  }
}, 100);
